...
export function Sheet({ db, limit }) {
  let totalcount = 0
  ...
  return (
    <div className={'bg-white sm:rounded-md p-4 h-full hover:no-underline'}>
      <div className=" rounded-lg inline-block w-full hover:no-underline ">
      ...
      <table className="w-full table border-transparent">
        <tbody className="w-full table border-none">
          {db.completed.reverse().map((props, i) => (
            ...
            <tr key={i} className="">
              ...
              <td className="   border px-4 py-2">
                <svg
                  className="hidden md:inline-block flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400 float-left "
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
